...
module TK = struct
  ...
  let rec var_avoids_bin_range _ _ _ = Fail (* TODO: get rid of *)
...
= struct
  ...
  let zeros shape = Arr.create shape Float.neg_infinity
  let ones shape = Arr.create shape Float.infinity
...
module TensorTK(TS : TensorSemiring) = struct
  ...
  let var_possible_states (v : var) : TS.arr list =
    let s = TK.adt_size (snd v) in
    List.init s (unit_basis s)

  (* enforce two variables to have a target value *)
  ...
  let rec eval_tk (exp : tk) (env : adt list) (rels : comp_rel list) : TS.arr =
    ...
    | Lefto (ab, a) ->
        ...
        gen_unify ab a env
          (fun va -> vec_kron_sum va (TS.zeros [|TK.adt_size bt|]))
    | Righto (ab, b) ->
        (* check that right side matches *)
        let (_, Sum (at, _bt)) = ab in
        gen_unify ab b env
          (fun vb -> vec_kron_sum (TS.zeros [|TK.adt_size at|]) vb)
    | Pairo (ab, a, b) ->
        (* check that a and b match *)
        let (_, Prod (at, bt)) = ab in
        let a_check =
          gen_unify ab a env (fun va -> vec_kron_prod va (TS.ones [|TK.adt_size bt|]))
        in
        let b_check =
          gen_unify ab b env (fun vb -> vec_kron_prod (TS.ones [|TK.adt_size at|]) vb)
        in
...
module MaxAddTKEx = struct
  ...
  let ex_var_avoids_bin_range : tk_prgm = [
    { name = "maino";
      args = [bin_adt 4];
      body = var_avoids_bin_range (0, bin_adt 4) 1 4 }]
  ...
  let examples = [
    ...
    ("ex_var_avoids_val_zero", ex_var_avoids_val_zero);
    ("ex_var_avoids_val_one", ex_var_avoids_val_one);
    ("ex_var_avoids_val_two", ex_var_avoids_val_two);
    ("ex_var_avoids_bin_range", ex_var_avoids_bin_range);
    ("ex_var_has_val_zero_comp", ex_var_has_val_zero_comp);
    ("ex_var_has_val_one_comp", ex_var_has_val_one_comp);
    ("ex_var_has_val_two_comp", ex_var_has_val_two_comp);
  ]
